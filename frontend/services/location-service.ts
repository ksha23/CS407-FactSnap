 ...
 */
export async function fetchPlaceAutocomplete(input: string): Promise<Array<{ place_id: string; description: string }>> {
  try {
    ...
    if (!res.ok) {
      console.warn(
        '[Places] Autocomplete HTTP error:',
        res.status,
        res.statusText
      );
      return [];
    ...
    console.debug('[Places] Autocomplete response status:', json.status, 'error_message:', json.error_message);
    if (json.status !== 'OK') {
      // If zero results, return empty list; otherwise log for debugging
      if (json.status !== 'ZERO_RESULTS') {
        console.warn('[Places] Autocomplete returned non-OK status', json.status, json.error_message || '');
      }
      return [];
    }
    const preds = (json.predictions || []).map((p: any) => ({ place_id: p.place_id, description: p.description }));
