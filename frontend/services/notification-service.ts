    ...
    if (location) {
      ...
      try {
        ...
        if (fetchNearbyLocationsCallback) {
          ...
          const key = ['backgroundNearby', currentCoords.latitude, currentCoords.longitude, NOTIFICATION_RADIUS_MILES];
          try {
            ...
            console.error('Error fetching nearby locations via queryClient in background task:', err);
            nearbyLocations = [];
          }
        }
        ...
        if (nearbyLocations.length > 0) {
          ...
          await sendNearbyLocationNotification(nearbyLocations.length, nearbyLocations[0]);
        }
      } catch (error) {
...
export async function checkNearbyLocations(
  ...
  try {
    ...
    const nearbyLocations = await fetchCallback(currentCoords, NOTIFICATION_RADIUS_MILES);
